module dragon.battle0 {
	export class Buff {
		...
		public attrPer(attr: model.Attribute) {
			return this.$attrsPer[attr] || 0;
		}
		public attrFixed(attr: model.Attribute) {
		...
		public constructor(creature: Unit, buffid: number, lv: number) {
			...
			for (var i = 0; i < this.buffdata.AttrTypeList.length; ++i) {
				var attr = model.Attribute[this.buffdata.AttrTypeList[i]];
		...
		public isEffective() {
			...
			if (this.buffdata.HPCondition > 0) {
				return this.creature.hp / this.creature.baseAttr(model.Attribute.MaxHP) < this.buffdata.HPCondition / 100;
			}
			return true;
		}
